...
class ShortCorridor:
    ...
    def step(self, go_right: bool) -> (int, bool):
        """
        @param self:
        @param go_right: bool, choose if go right
        @return: (reward, ifTermineral)
        """
...
class ReinforceAgent:
    ...
    def get_pi(self):
        ...
        # make sure the action with small probability no fewer than epsilon?
        imin = np.argmin(pmf)
        epsilon = 0.05
        if pmf[imin] < epsilon:
            pmf[:] = 1 - epsilon
    ...
    def get_p_right(self):
        """
        get the probability of choosing action right
        @return:
        """
    ...
    def choose_action(self, reward):
        """
        get pmf of policy, based on the probability of choosing going right
        select a random number, if it's smaller than the action right prob
        then go right, otherwise go left
        @param reward:
        @return:
        """
